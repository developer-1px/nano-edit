...
await withBrowserRegression('nano-edit-nano2-demo-', async ({ browser, url }) => {
  ...
  assert.equal(await nano2ExampleHref(browser, 'tiptap-iframe'), '/nano2/tiptap-iframe')
  assert.equal(await nano2ExampleHref(browser, 'tiptap-interactive-views'), '/nano2/tiptap-interactive-views')
  ...
  await clickTarget(browser, '.nano2-example-link[data-example-id="tiptap-interactive-views"]')
  await waitForExpression(browser, 'location.pathname === "/nano2/tiptap-interactive-views"')
  await waitForExpression(browser, 'document.querySelector(".nano2-example-title")?.textContent.includes("Tiptap Interactive Views")')
  await waitForExpression(browser, `document.querySelector('.nano2')?.dataset.profile === 'interactive'`)
  await waitForExpression(browser, `document.querySelector('.nano2 .nano2-interactive-view[data-id="nano2-interactive-counter"]')?.dataset.count === '2'`)

  await clickTarget(browser, '.nano2 .nano2-interactive-view[data-id="nano2-interactive-counter"] .nano2-interactive-button[aria-label="Increment component value"]')
  await waitForExpression(browser, `document.querySelector('.nano2 .nano2-interactive-view[data-id="nano2-interactive-counter"]')?.dataset.count === '3'`)
  await appendText(browser, '.nano2 .nano2-interactive-view[data-id="nano2-interactive-counter"] .nano2-interactive-label', ' edited')
  await waitForExpression(browser, `document.querySelector('.nano2 .nano2-interactive-view[data-id="nano2-interactive-counter"]')?.dataset.label === 'Interactive node view edited'`)

  await wait(160)
  const storedInteractiveViews = await storedNano2Document(browser, 'tiptap-interactive-views')
  const storedInteractiveBlock = storedInteractiveViews.blocks.find((block) => block.id === 'nano2-interactive-counter')
  assert(storedInteractiveBlock)
  assert.equal(storedInteractiveBlock.type, 'nano2.interactive-view')
  assert.equal(storedInteractiveBlock.data.count, 3)
  assert.equal(storedInteractiveBlock.data.label, 'Interactive node view edited')
  assert.equal('component' in storedInteractiveBlock.data, false)
  assert.equal('dom' in storedInteractiveBlock.data, false)

  await clickTarget(browser, '.nano2-example-link[data-example-id="tiptap-figure"]')
  await waitForExpression(browser, 'location.pathname === "/nano2/tiptap-figure"')
  await waitForExpression(browser, 'document.querySelector(".nano2-example-title")?.textContent.includes("Tiptap Figure")')
  await waitForExpression(browser, `document.querySelector('.nano2')?.dataset.profile === 'figure'`)
  await waitForExpression(browser, `document.querySelector('.nano2 .nano2-figure-block[data-id="nano2-figure-image"]')?.dataset.kind === 'image'`)
  await waitForExpression(browser, `document.querySelector('.nano2 .nano2-figure-block[data-id="nano2-figure-table"]')?.dataset.kind === 'table'`)

  await appendText(browser, '.nano2 .nano2-figure-block[data-id="nano2-figure-image"] .nano2-figure-caption', ' edited')
  await waitForExpression(browser, `document.querySelector('.nano2 .nano2-figure-block[data-id="nano2-figure-image"]')?.dataset.caption === 'Image figure caption edited'`)
  await wait(160)
  const storedFigure = await storedNano2Document(browser, 'tiptap-figure')
  const storedImageFigure = storedFigure.blocks.find((block) => block.id === 'nano2-figure-image')
  const storedTableFigure = storedFigure.blocks.find((block) => block.id === 'nano2-figure-table')
  assert(storedImageFigure)
  assert(storedTableFigure)
  assert.equal(storedImageFigure.type, 'nano2.figure')
  assert.equal(storedImageFigure.data.kind, 'image')
  assert.equal(storedImageFigure.data.caption, 'Image figure caption edited')
  assert.equal(storedImageFigure.text, 'Image figure caption edited')
  assert.equal(storedTableFigure.data.kind, 'table')
  assert.equal(storedTableFigure.data.table.rows.length, 3)
  assert.equal('figcaption' in storedImageFigure.data, false)
  assert.equal('dom' in storedImageFigure.data, false)

  await clickTarget(browser, '.nano2-example-link[data-example-id="tiptap-iframe"]')
  await waitForExpression(browser, 'location.pathname === "/nano2/tiptap-iframe"')
  await waitForExpression(browser, 'document.querySelector(".nano2-example-title")?.textContent.includes("Tiptap iFrame")')
  await waitForExpression(browser, `document.querySelector('.nano2')?.dataset.profile === 'iframe'`)
  await waitForExpression(browser, `document.querySelector('.nano2 .nano2-iframe-block[data-id="nano2-iframe-embed"]')?.dataset.src === 'https://example.com/nano2-iframe'`)
  await waitForExpression(browser, `document.querySelector('.nano2 .nano2-iframe-frame')?.getAttribute('src') === 'https://example.com/nano2-iframe'`)

  await clickTarget(browser, '.nano2 .nano2-iframe-button[aria-label="Use sample iframe attrs"]')
  await waitForExpression(browser, `document.querySelector('.nano2 .nano2-iframe-block[data-id="nano2-iframe-embed"]')?.dataset.src === 'https://example.com/nano2-iframe-updated'`)
  await wait(160)
  const storedIFrame = await storedNano2Document(browser, 'tiptap-iframe')
  const storedIFrameBlock = storedIFrame.blocks.find((block) => block.id === 'nano2-iframe-embed')
  assert(storedIFrameBlock)
  assert.equal(storedIFrameBlock.type, 'nano2.iframe')
  assert.equal(storedIFrameBlock.data.src, 'https://example.com/nano2-iframe-updated')
  assert.equal(storedIFrameBlock.data.title, 'Updated Nano2 iframe')
  assert.equal(storedIFrameBlock.text, 'Updated Nano2 iframe')
  assert.equal('contentWindow' in storedIFrameBlock.data, false)
  assert.equal('dom' in storedIFrameBlock.data, false)

  await clickTarget(browser, '.nano2-example-link[data-example-id="tiptap-linting"]')
  await waitForExpression(browser, 'location.pathname === "/nano2/tiptap-linting"')
  await waitForExpression(browser, 'document.querySelector(".nano2-example-title")?.textContent.includes("Tiptap Linting")')
  await waitForExpression(browser, `document.querySelector('.nano2')?.dataset.profile === 'linting'`)
  await waitForExpression(browser, `Boolean(document.querySelector('.nano2 [data-lint-rule="typo-teh"]'))`)
  await waitForExpression(browser, `Boolean(document.querySelector('.nano2 [data-lint-rule="repeated-word"]'))`)

  await clickTarget(browser, '.nano2 [data-lint-rule="typo-teh"]')
  await waitForExpression(browser, `!document.querySelector('.nano2 [data-lint-rule="typo-teh"]')`)
  await waitForExpression(browser, `document.querySelector('.nano2 [data-id="nano2-linting-target"]')?.textContent.includes('This is the linting')`)
  await wait(160)
  const storedLinting = await storedNano2Document(browser, 'tiptap-linting')
  const storedLintingBlock = storedLinting.blocks.find((block) => block.id === 'nano2-linting-target')
  assert(storedLintingBlock)
  assert.equal(storedLintingBlock.text.includes('teh'), false)
  assert.equal(storedLintingBlock.text.includes('the linting'), true)
  assert.equal('diagnostics' in storedLintingBlock, false)
