...
type CommandBlockOption = BlockOption & { template: BlockTemplate }
...
export function blockCommands(options: NanoCommandsOptions): NanoCommand[] {
  ...
  return (options.blockOptions ?? defaultBlockOptions)
    .filter((option): option is CommandBlockOption => option.template !== undefined)
    .map((option): NanoCommand => ({
      id: `block:${option.id}`,
      title: option.title,
      hint: option.label,
