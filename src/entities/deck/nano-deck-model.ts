...
function validateSlideRegions(ctx, slide, slideIndex, blockIds) {
    ...
    for (const [regionIndex, region] of slide.regions.entries()) {
        ...
        for (const [blockIndex, block] of region.blocks.entries()) {
            addUniqueIdIssue(ctx, blockIds, block.id, ['slides', slideIndex, 'regions', regionIndex, 'blocks', blockIndex, 'id'], 'Duplicate deck block id');
...
function addUniqueIdIssue(ctx, seen, id, path, label) {
    if (!seen.has(id)) {
        seen.add(id);
        return;
    }
...
function validateTextBlockMarks(ctx, block, path) {
